...
function MissIon179(UserWorldId,QuestId, CzId, RwJdId)
  ...
  elseif CzId==5 then
      local QuestLevel=GetQuestLevel(UserWorldId,QuestId)
      if QuestLevel==1 then
              local weiz900000522=GetPackage(UserWorldId)
              if weiz900000522==-1 then
                  SendSysMsg(UserWorldId,"There is no space in the equipment column", 9, "SystemMessage")
                  SendMissionMsg(UserWorldId,QuestId, 12, RwJdId)
                  return 
              else
                  AddItme(UserWorldId,900000522,weiz900000522,1)
              end
      end
