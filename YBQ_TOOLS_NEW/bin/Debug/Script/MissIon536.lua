...
function MissIon536(UserWorldId,QuestId, CzId, RwJdId)
  ...
  if CzId==1 then
      if Player.Player_FLD_LEVEL<55 then
          ...
          if QuestLevel==1 then
              ...
              local  bool900000690 = GetQuestItme(UserWorldId, 900000690, 1)
              if bool900000690 then
                  AddQuestItme(UserWorldId,900000690,0)
              else
                  SendMissionMsg(UserWorldId,QuestId, 12, RwJdId)
              end
          else
  ...
  elseif CzId==5 then
      ...
      if QuestLevel==2 then
              local weiz800000002=GetPackage(UserWorldId)
              if weiz800000002==-1 then
                  SendSysMsg(UserWorldId,"There is no space in the equipment column", 9, "SystemMessage")
                  SendMissionMsg(UserWorldId,QuestId, 12, RwJdId)
                  return 
              else
                  AddItme(UserWorldId,800000002,weiz800000002,2)
              end
              ...
              if weiz909000001==-1 then
                  ...
                  AddItme(UserWorldId,909000001,weiz909000001,1500067)
              end
              local weiz900000090=GetPackage(UserWorldId)
              if weiz900000090==-1 then
                  SendSysMsg(UserWorldId,"There is no space in the equipment column", 9, "SystemMessage")
                  SendMissionMsg(UserWorldId,QuestId, 12, RwJdId)
                  return 
              else
                  AddItme(UserWorldId,900000090,weiz900000090,1)
              end
      end
