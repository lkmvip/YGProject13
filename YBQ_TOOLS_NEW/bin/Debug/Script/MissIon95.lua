...
function MissIon95(UserWorldId,QuestId, CzId, RwJdId)
  ...
  elseif CzId==5 then
      ...
      if QuestLevel==3 then
              ...
              local weiz900000275=GetPackage(UserWorldId)
              if weiz900000275==-1 then
                  SendSysMsg(UserWorldId,"There is no space in the equipment column", 9, "SystemMessage")
                  SendMissionMsg(UserWorldId,QuestId, 12, RwJdId)
                  return 
              else
                  AddItme(UserWorldId,900000275,weiz900000275,1)
              end
