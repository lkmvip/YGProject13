...
function MissIon228(UserWorldId,QuestId, CzId, RwJdId)
  ...
  elseif CzId==5 then
      ...
      if QuestLevel==2 then
              local weiz909000015=GetPackage(UserWorldId)
              if weiz909000015==-1 then
                  SendSysMsg(UserWorldId,"There is no space in the equipment column", 9, "SystemMessage")
                  SendMissionMsg(UserWorldId,QuestId, 12, RwJdId)
                  return 
              else
                  AddItme(UserWorldId,909000015,weiz909000015,1)
              end
      end
