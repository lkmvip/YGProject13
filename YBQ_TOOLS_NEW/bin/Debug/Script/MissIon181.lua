...
function MissIon181(UserWorldId,QuestId, CzId, RwJdId)
  ...
  if CzId==1 then
      if Player.Player_FLD_LEVEL<100 then
          ...
          if QuestLevel==1 then
              ...
              local  bool900000518 = GetQuestItme(UserWorldId, 900000518, 10)
              if bool900000518 then
                  AddQuestItme(UserWorldId,900000518,0)
              else
                  SendMissionMsg(UserWorldId,QuestId, 12, RwJdId)
              end
          else
  ...
  elseif CzId==5 then
      ...
      if QuestLevel==2 then
              local weiz900000523=GetPackage(UserWorldId)
              if weiz900000523==-1 then
                  SendSysMsg(UserWorldId,"There is no space in the equipment column", 9, "SystemMessage")
                  SendMissionMsg(UserWorldId,QuestId, 12, RwJdId)
                  return 
              else
                  AddItme(UserWorldId,900000523,weiz900000523,1)
              end
      end
