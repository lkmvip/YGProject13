...
function MissIon1009(UserWorldId,QuestId, CzId, RwJdId)
  ...
  if CzId==1 then
      if Player.Player_FLD_LEVEL<0 then
          ...
          if QuestLevel==1 then
              ...
              local  bool900000421 = GetQuestItme(UserWorldId, 900000421, 20)
              if bool900000421 then
                  AddQuestItme(UserWorldId,900000421,0)
              else
                  SendMissionMsg(UserWorldId,QuestId, 12, RwJdId)
              end
          else
  ...
  elseif CzId==5 then
      ...
      if QuestLevel==2 then
              local weiz800000030=GetPackage(UserWorldId)
              if weiz800000030==-1 then
              ...
              local weiz800000032=GetPackage(UserWorldId)
              if weiz800000032==-1 then
                  SendSysMsg(UserWorldId,"There is no space in the equipment column", 9, "SystemMessage")
                  SendMissionMsg(UserWorldId,QuestId, 12, RwJdId)
                  return 
              else
