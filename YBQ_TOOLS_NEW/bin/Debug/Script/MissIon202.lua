...
function MissIon202(UserWorldId,QuestId, CzId, RwJdId)
  ...
  elseif CzId==5 then
      ...
      if QuestLevel==2 then
              local weiz909000008=GetPackage(UserWorldId)
              if weiz909000008==-1 then
                  SendSysMsg(UserWorldId,"There is no space in the equipment column", 9, "SystemMessage")
                  SendMissionMsg(UserWorldId,QuestId, 12, RwJdId)
                  return 
              else
                  AddItme(UserWorldId,909000008,weiz909000008,1)
              end
      end
