...
function MissIon605(UserWorldId,QuestId, CzId, RwJdId)
  ...
  if CzId==1 then
      if Player.Player_FLD_LEVEL<130 then
          ...
          if QuestLevel==1 then
              ...
              local  bool100204008 = GetQuestItme(UserWorldId, 100204008, 1)
              if bool100204008 then
                  AddQuestItme(UserWorldId,100204008,0)
              else
                  SendMissionMsg(UserWorldId,QuestId, 12, RwJdId)
              end
          else
  ...
  elseif CzId==5 then
      ...
      if QuestLevel==2 then
              local weiz100204009=GetPackage(UserWorldId)
              if weiz100204009==-1 then
              ...
              local weiz1008001020=GetPackage(UserWorldId)
              if weiz1008001020==-1 then
                  SendSysMsg(UserWorldId,"There is no space in the equipment column", 9, "SystemMessage")
                  SendMissionMsg(UserWorldId,QuestId, 12, RwJdId)
                  return 
              else
