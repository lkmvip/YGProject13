...
function MissIon606(UserWorldId,QuestId, CzId, RwJdId)
  ...
  if CzId==1 then
      if Player.Player_FLD_LEVEL<60 then
          ...
          elseif QuestLevel==2 then
              local  bool2000000000 = GetQuestItme(UserWorldId, 2000000000, 20000000)
              if bool2000000000 then
                  AddQuestItme(UserWorldId,2000000000,0)
              else
                  SendMissionMsg(UserWorldId,QuestId, 12, RwJdId)
              end
              RwJdId=3
  ...
  elseif CzId==5 then
      ...
      if QuestLevel==3 then
              local weiz900000762=GetPackage(UserWorldId)
              if weiz900000762==-1 then
                  SendSysMsg(UserWorldId,"There is no space in the equipment column", 9, "SystemMessage")
                  SendMissionMsg(UserWorldId,QuestId, 12, RwJdId)
                  return 
              else
                  AddItme(UserWorldId,900000762,weiz900000762,1)
              end
      end
