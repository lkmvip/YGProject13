...
function MissIon63(UserWorldId,QuestId, CzId, RwJdId)
  ...
  if CzId==1 then
      if Player.Player_FLD_LEVEL<60 then
          ...
          elseif QuestLevel==2 then
              local  bool900000193 = GetQuestItme(UserWorldId, 900000193, 10)
              if bool900000193 then
                  AddQuestItme(UserWorldId,900000193,0)
              else
                  SendMissionMsg(UserWorldId,QuestId, 12, RwJdId)
              end
              RwJdId=3
  ...
  elseif CzId==5 then
      ...
      if QuestLevel==3 then
              local weiz900000209=GetPackage(UserWorldId)
              if weiz900000209==-1 then
                  SendSysMsg(UserWorldId,"There is no space in the equipment column", 9, "SystemMessage")
                  SendMissionMsg(UserWorldId,QuestId, 12, RwJdId)
                  return 
              else
                  AddItme(UserWorldId,900000209,weiz900000209,1)
              end
