...
function MissIon377(UserWorldId,QuestId, CzId, RwJdId)
  ...
  elseif CzId==5 then
      ...
      if QuestLevel==2 then
              local weiz900504030=GetPackage(UserWorldId)
              if weiz900504030==-1 then
                  SendSysMsg(UserWorldId,"There is no space in the equipment column", 9, "SystemMessage")
                  SendMissionMsg(UserWorldId,QuestId, 12, RwJdId)
                  return 
              else
                  AddItme(UserWorldId,900504030,weiz900504030,1)
              end
