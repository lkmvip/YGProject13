...
function MissIon226(UserWorldId,QuestId, CzId, RwJdId)
  ...
  if CzId==1 then
      if Player.Player_FLD_LEVEL<60 then
          ...
          if QuestLevel==1 then
              ...
              local  bool900000600 = GetQuestItme(UserWorldId, 900000600, 10)
  ...
  elseif CzId==5 then
      ...
      if QuestLevel==2 then
              local weiz909000016=GetPackage(UserWorldId)
              if weiz909000016==-1 then
                  SendSysMsg(UserWorldId,"There is no space in the equipment column", 9, "SystemMessage")
                  SendMissionMsg(UserWorldId,QuestId, 12, RwJdId)
                  return 
              else
                  AddItme(UserWorldId,909000016,weiz909000016,1)
              end
      end
