...
function MissIon131(UserWorldId,QuestId, CzId, RwJdId)
  ...
  if CzId==1 then
      if Player.Player_FLD_LEVEL<90 then
          ...
          elseif QuestLevel==3 then
              local  bool900000347 = GetQuestItme(UserWorldId, 900000347, 7)
              if bool900000347 then
                  AddQuestItme(UserWorldId,900000347,0)
              else
                  SendMissionMsg(UserWorldId,QuestId, 12, RwJdId)
              end
              RwJdId=4
  ...
  elseif CzId==5 then
      ...
      if QuestLevel==5 then
              local weiz909000007=GetPackage(UserWorldId)
              if weiz909000007==-1 then
                  SendSysMsg(UserWorldId,"There is no space in the equipment column", 9, "SystemMessage")
                  SendMissionMsg(UserWorldId,QuestId, 12, RwJdId)
                  return 
              else
                  AddItme(UserWorldId,909000007,weiz909000007,1)
              end
