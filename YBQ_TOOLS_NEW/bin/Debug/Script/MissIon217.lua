...
function MissIon217(UserWorldId,QuestId, CzId, RwJdId)
  ...
  if CzId==1 then
      if Player.Player_FLD_LEVEL<0 then
          ...
          if QuestLevel==1 then
              ...
              local  bool1000000447 = GetQuestItme(UserWorldId, 1000000447, 1)
              if bool1000000447 then
                  AddQuestItme(UserWorldId,1000000447,0)
              else
                  SendMissionMsg(UserWorldId,QuestId, 12, RwJdId)
              end
          else
  ...
  elseif CzId==5 then
      ...
      if QuestLevel==2 then
              ...
              local weiz1000000783=GetPackage(UserWorldId)
              if weiz1000000783==-1 then
                  SendSysMsg(UserWorldId,"There is no space in the equipment column", 9, "SystemMessage")
                  SendMissionMsg(UserWorldId,QuestId, 12, RwJdId)
                  return 
              else
