...
--Ten nhiem vu: ([GM] [??? 1? ??] ??????? ??!!) --- So buoc: (1)
function MissIon475(UserWorldId,QuestId, StepQuest, StatusQuest)
  ...
  if StepQuest==1 then
      if Player.Player_FLD_LEVEL<10 then 
          ...
          if QuestLevel==1 then  --neu dang o buoc 1
              ...
              local  bool900000788 = GetQuestItme(UserWorldId, 900000788, 1)  --kiem tra vat pham nhiem vu
              if bool900000788 then --du yeu cau
                  AddQuestItme(UserWorldId,900000788,0) --xoa item nhiem vu
      ...
      if Player.Player_FLD_LEVEL<10 then  
         ...
         if QuestLevel==2 then 
            SendMissionMsg(UserWorldId,QuestId, 22, StatusQuest) 
                  ...
                  AddQuestItme(UserWorldId,900000788,1) --them vat pham nhiem vu cho buoc tiep theo
            StatusQuest=1
      ...
      local controng=GetPackages(UserWorldId,2)  --vi tri con trong
        if controng.Count < 2 then  --neu thung do day 
          ...
          return 
                  ...
                  AddMoneyToCharacter(UserWorldId,10000) 
                  SendSysMsg(UserWorldId,"Nhan duoc 10000 tien vang!", 9, "He thong")
              --tang thuong
              local weiz800000006=GetPackage(UserWorldId)  --vi tri con trong
                  AddItme(UserWorldId,800000006,weiz800000006,1) 
        end 
