...
function MissIon357(UserWorldId,QuestId, CzId, RwJdId)
  ...
  if CzId==1 then
      if Player.Player_FLD_LEVEL<122 then
          ...
          if QuestLevel==1 then
              ...
              local  bool900000619 = GetQuestItme(UserWorldId, 900000619, 1)
              if bool900000619 then
                  AddQuestItme(UserWorldId,900000619,0)
              else
                  SendMissionMsg(UserWorldId,QuestId, 12, RwJdId)
              end
          else
  ...
  elseif CzId==5 then
      ...
      if QuestLevel==2 then
              ...
              local weiz1000000792=GetPackage(UserWorldId)
              if weiz1000000792==-1 then
                  SendSysMsg(UserWorldId,"There is no space in the equipment column", 9, "SystemMessage")
                  SendMissionMsg(UserWorldId,QuestId, 12, RwJdId)
                  return 
              else
