...
function MissIon93(UserWorldId,QuestId, CzId, RwJdId)
  ...
  if CzId==1 then
      if Player.Player_FLD_LEVEL<72 then
          ...
          if QuestLevel==1 then
              ...
              local  bool900000269 = GetQuestItme(UserWorldId, 900000269, 10)
              if bool900000269 then
                  AddQuestItme(UserWorldId,900000269,0)
              else
                  SendMissionMsg(UserWorldId,QuestId, 12, RwJdId)
              end
              local  bool900000270 = GetQuestItme(UserWorldId, 900000270, 5)
              if bool900000270 then
                  AddQuestItme(UserWorldId,900000270,0)
              else
                  SendMissionMsg(UserWorldId,QuestId, 12, RwJdId)
              end
          else
  ...
  elseif CzId==5 then
      ...
      if QuestLevel==2 then
              ...
              local weiz900000271=GetPackage(UserWorldId)
              if weiz900000271==-1 then
                  SendSysMsg(UserWorldId,"There is no space in the equipment column", 9, "SystemMessage")
                  SendMissionMsg(UserWorldId,QuestId, 12, RwJdId)
                  return 
              else
                  AddItme(UserWorldId,900000271,weiz900000271,1)
              end
