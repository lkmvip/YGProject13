...
--Ten nhiem vu: ([GM] [????? 5? ??]) --- So buoc: (1)
function MissIon634(UserWorldId,QuestId, StepQuest, StatusQuest)
  ...
  if StepQuest==1 then
      if Player.Player_FLD_LEVEL<100 then 
          ...
          if QuestLevel==1 then  --neu dang o buoc 1
              ...
              local  bool430203006 = GetQuestItme(UserWorldId, 430203006, 1)  --kiem tra vat pham nhiem vu
              local  bool900000762 = GetQuestItme(UserWorldId, 900000762, 1)  --kiem tra vat pham nhiem vu
              if bool430203006 then --du yeu cau
              if bool900000762 then --du yeu cau
                  AddQuestItme(UserWorldId,430203006,0) --xoa item nhiem vu
      ...
      if Player.Player_FLD_LEVEL<100 then  
         ...
         if QuestLevel==2 then 
            SendMissionMsg(UserWorldId,QuestId, 22, StatusQuest) 
                  ...
                  AddQuestItme(UserWorldId,430203006,1) --them vat pham nhiem vu cho buoc tiep theo
                  AddQuestItme(UserWorldId,900000762,1) --them vat pham nhiem vu cho buoc tiep theo
            StatusQuest=1
      ...
      local controng=GetPackages(UserWorldId,1)  --vi tri con trong
        if controng.Count < 1 then  --neu thung do day 
          ...
          return 
              ...
              --tang thuong
              local weiz430203007=GetPackage(UserWorldId)  --vi tri con trong
                  AddItme(UserWorldId,430203007,weiz430203007,1) 
        end 
