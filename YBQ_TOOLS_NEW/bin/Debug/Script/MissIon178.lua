...
function MissIon178(UserWorldId,QuestId, CzId, RwJdId)
  ...
  if CzId==1 then
      if Player.Player_FLD_LEVEL<100 then
          ...
          elseif QuestLevel==2 then
              local  bool900000515 = GetQuestItme(UserWorldId, 900000515, 1)
              if bool900000515 then
                  AddQuestItme(UserWorldId,900000515,0)
              else
                  SendMissionMsg(UserWorldId,QuestId, 12, RwJdId)
              end
              RwJdId=3
  ...
  elseif CzId==5 then
      ...
      if QuestLevel==5 then
              local weiz900000521=GetPackage(UserWorldId)
              if weiz900000521==-1 then
                  SendSysMsg(UserWorldId,"There is no space in the equipment column", 9, "SystemMessage")
                  SendMissionMsg(UserWorldId,QuestId, 12, RwJdId)
                  return 
              else
                  AddItme(UserWorldId,900000521,weiz900000521,1)
              end
      end
