...
function MissIon253(UserWorldId,QuestId, CzId, RwJdId)
  ...
  if CzId==1 then
      if Player.Player_FLD_LEVEL<129 then
          ...
          if QuestLevel==1 then
              ...
              local  bool900000560 = GetQuestItme(UserWorldId, 900000560, 50)
              if bool900000560 then
                  AddQuestItme(UserWorldId,900000560,0)
              else
                  SendMissionMsg(UserWorldId,QuestId, 12, RwJdId)
              end
          else
  ...
  elseif CzId==5 then
      ...
      if QuestLevel==2 then
              ...
              if weiz800000029==-1 then
                  ...
                  AddItme(UserWorldId,800000029,weiz800000029,2)
              end
              local weiz900000592=GetPackage(UserWorldId)
              if weiz900000592==-1 then
                  SendSysMsg(UserWorldId,"There is no space in the equipment column", 9, "SystemMessage")
                  SendMissionMsg(UserWorldId,QuestId, 12, RwJdId)
                  return 
              else
                  AddItme(UserWorldId,900000592,weiz900000592,1)
              end
