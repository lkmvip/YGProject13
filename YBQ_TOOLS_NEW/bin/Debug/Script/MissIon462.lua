...
function MissIon462(UserWorldId,QuestId, CzId, RwJdId)
  ...
  elseif CzId==5 then
      ...
      if QuestLevel==2 then
              ...
              if weiz909000004==-1 then
                  ...
                  AddItme(UserWorldId,909000004,weiz909000004,2000000)
              end
              local weiz900000155=GetPackage(UserWorldId)
              if weiz900000155==-1 then
                  SendSysMsg(UserWorldId,"There is no space in the equipment column", 9, "SystemMessage")
                  SendMissionMsg(UserWorldId,QuestId, 12, RwJdId)
                  return 
              else
                  AddItme(UserWorldId,900000155,weiz900000155,1)
              end
      end
