...
function MissIon389(UserWorldId,QuestId, CzId, RwJdId)
  ...
  if CzId==1 then
      if Player.Player_FLD_LEVEL<140 then
          ...
          if QuestLevel==1 then
              ...
              local  bool1008000388 = GetQuestItme(UserWorldId, 1008000388, 1)
              if bool1008000388 then
                  AddQuestItme(UserWorldId,1008000388,0)
              else
                  SendMissionMsg(UserWorldId,QuestId, 12, RwJdId)
              end
          else
  ...
  elseif CzId==5 then
      ...
      if QuestLevel==2 then
              local weiz909000021=GetPackage(UserWorldId)
              if weiz909000021==-1 then
                  SendSysMsg(UserWorldId,"There is no space in the equipment column", 9, "SystemMessage")
                  SendMissionMsg(UserWorldId,QuestId, 12, RwJdId)
                  return 
              else
                  AddItme(UserWorldId,909000021,weiz909000021,100)
              end
      end
