...
function MissIon574(UserWorldId,QuestId, CzId, RwJdId)
  ...
  if CzId==1 then
      if Player.Player_FLD_LEVEL<145 then
          ...
          if QuestLevel==1 then
              ...
              local  bool900000730 = GetQuestItme(UserWorldId, 900000730, 10)
              if bool900000730 then
                  AddQuestItme(UserWorldId,900000730,0)
              else
                  SendMissionMsg(UserWorldId,QuestId, 12, RwJdId)
              end
          else
  ...
  elseif CzId==5 then
      ...
      if QuestLevel==2 then
              ...
              local weiz800000028=GetPackage(UserWorldId)
              if weiz800000028==-1 then
                  SendSysMsg(UserWorldId,"There is no space in the equipment column", 9, "SystemMessage")
                  SendMissionMsg(UserWorldId,QuestId, 12, RwJdId)
                  return 
              else
