...
function MissIon555(UserWorldId,QuestId, CzId, RwJdId)
  ...
  if CzId==1 then
      if Player.Player_FLD_LEVEL<45 then
          ...
          if QuestLevel==1 then
              ...
              local  bool900000711 = GetQuestItme(UserWorldId, 900000711, 10)
              if bool900000711 then
                  AddQuestItme(UserWorldId,900000711,0)
              else
                  SendMissionMsg(UserWorldId,QuestId, 12, RwJdId)
              end
          else
  ...
  elseif CzId==5 then
      ...
      if QuestLevel==2 then
              ...
              if weiz909000001==-1 then
                  ...
                  AddItme(UserWorldId,909000001,weiz909000001,420000)
              end
              local weiz1008000019=GetPackage(UserWorldId)
              if weiz1008000019==-1 then
                  SendSysMsg(UserWorldId,"There is no space in the equipment column", 9, "SystemMessage")
                  SendMissionMsg(UserWorldId,QuestId, 12, RwJdId)
                  return 
              else
