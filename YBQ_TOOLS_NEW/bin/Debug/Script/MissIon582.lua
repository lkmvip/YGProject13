...
function MissIon582(UserWorldId,QuestId, CzId, RwJdId)
  ...
  if CzId==1 then
      if Player.Player_FLD_LEVEL<150 then
          ...
          if QuestLevel==1 then
              ...
              local  bool900000738 = GetQuestItme(UserWorldId, 900000738, 10)
              if bool900000738 then
                  AddQuestItme(UserWorldId,900000738,0)
              else
                  SendMissionMsg(UserWorldId,QuestId, 12, RwJdId)
              end
          else
  ...
  elseif CzId==5 then
      ...
      if QuestLevel==2 then
              local weiz800000060=GetPackage(UserWorldId)
              if weiz800000060==-1 then
              ...
              local weiz800000061=GetPackage(UserWorldId)
              if weiz800000061==-1 then
                  SendSysMsg(UserWorldId,"There is no space in the equipment column", 9, "SystemMessage")
                  SendMissionMsg(UserWorldId,QuestId, 12, RwJdId)
                  return 
              else
              ...
              if weiz909000001==-1 then
                  ...
                  AddItme(UserWorldId,909000001,weiz909000001,0)
              end
      end
