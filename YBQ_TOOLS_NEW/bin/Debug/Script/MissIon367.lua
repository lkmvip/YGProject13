...
function MissIon367(UserWorldId,QuestId, CzId, RwJdId)
  ...
  if CzId==1 then
      if Player.Player_FLD_LEVEL<60 then
          ...
          if QuestLevel==1 then
              ...
              local  bool900000635 = GetQuestItme(UserWorldId, 900000635, 10)
              if bool900000635 then
                  AddQuestItme(UserWorldId,900000635,0)
              else
                  SendMissionMsg(UserWorldId,QuestId, 12, RwJdId)
              end
          else
  ...
  elseif CzId==5 then
      ...
      if QuestLevel==2 then
              local weiz1000000531=GetPackage(UserWorldId)
              if weiz1000000531==-1 then
              ...
              local weiz1000000535=GetPackage(UserWorldId)
              if weiz1000000535==-1 then
                  SendSysMsg(UserWorldId,"There is no space in the equipment column", 9, "SystemMessage")
                  SendMissionMsg(UserWorldId,QuestId, 12, RwJdId)
                  return 
              else
