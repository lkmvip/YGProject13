...
function MissIon500(UserWorldId,QuestId, StepQuest, StatusQuest)
  ...
  if StepQuest==1 then
      if Player.Player_FLD_LEVEL<1 then 
          ...
          if QuestLevel==1 then  --neu dang o buoc 1
              ...
              local  bool900000651 = GetQuestItme(UserWorldId, 900000651, 1)  --kiem tra vat pham nhiem vu
              if bool900000651 then --du yeu cau
                  AddQuestItme(UserWorldId,900000651,0) --xoa item nhiem vu
      ...
      if Player.Player_FLD_LEVEL<1 then  
         ...
         if QuestLevel==2 then 
            SendMissionMsg(UserWorldId,QuestId, 22, StatusQuest) 
                  ...
                  AddQuestItme(UserWorldId,900000651,1) --them vat pham nhiem vu cho buoc tiep theo
            StatusQuest=1
      ...
      local controng=GetPackages(UserWorldId,3)  --vi tri con trong
        if controng.Count < 3 then  --neu thung do day 
          ...
          return 
              ...
              --tang thuong
              local weiz800002=GetPackage(UserWorldId)  --vi tri con trong
                  AddItme(UserWorldId,800002,weiz800002,1) 
              --tang thuong
              local weiz1000000902=GetPackage(UserWorldId)  --vi tri con trong
                  AddItme(UserWorldId,1000000902,weiz1000000902,5) 
                  AddExpToCharacter(UserWorldId,90) 
        end 
