...
--Ten nhiem vu: ([GM] [????? 9? ??]) --- So buoc: (1)
function MissIon638(UserWorldId,QuestId, StepQuest, StatusQuest)
  local Player = GetPlayer(UserWorldId)
--Cac buoc xu li nhiem vu
  if StepQuest==1 then
      if Player.Player_FLD_LEVEL<140 then 
          SendSysMsg(UserWorldId,"Ban phai dat cap do 140 moi co the nhan nhiem vu !", 9, "He thong")
          ...
          if QuestLevel==1 then  --neu dang o buoc 1
              ...
              local  bool430203010 = GetQuestItme(UserWorldId, 430203010, 1)  --kiem tra vat pham nhiem vu
              local  bool900000762 = GetQuestItme(UserWorldId, 900000762, 5)  --kiem tra vat pham nhiem vu
              local  bool200204010 = GetQuestItme(UserWorldId, 200204010, 1)  --kiem tra vat pham nhiem vu
              if bool430203010 then --du yeu cau
              if bool900000762 then --du yeu cau
              if bool200204010 then --du yeu cau
                  AddQuestItme(UserWorldId,430203010,0) --xoa item nhiem vu
                  AddQuestItme(UserWorldId,900000762,0) --xoa item nhiem vu
                  AddQuestItme(UserWorldId,200204010,0) --xoa item nhiem vu
      ...
      if Player.Player_FLD_LEVEL<140 then  
         ...
         if QuestLevel==2 then 
            SendMissionMsg(UserWorldId,QuestId, 22, StatusQuest) 
                  ...
                  AddQuestItme(UserWorldId,430203010,1) --them vat pham nhiem vu cho buoc tiep theo
 
-- VAT PHAM ROT RA KHI DANH QUAI --
-- ID VAT PHAM: ( 900000762 ) -- SO LUONG: ( 5 ) -- BUOC: ( 1 ) --
-- BAN DO: ( 101 ) -- X: ( 0 ) -- Y: ( 0 ) --
 
                  AddQuestItme(UserWorldId,200204010,1) --them vat pham nhiem vu cho buoc tiep theo
            StatusQuest=1
      ...
      local controng=GetPackages(UserWorldId,1)  --vi tri con trong
        if controng.Count < 1 then  --neu thung do day 
          ...
          return 
              ...
              --tang thuong
              local weiz430203012=GetPackage(UserWorldId)  --vi tri con trong
                  AddItme(UserWorldId,430203012,weiz430203012,1) 
        end 
