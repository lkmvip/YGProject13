...
--Ten nhiem vu: (?? ????бн) --- So buoc: (1)
function MissIon699(UserWorldId,QuestId, StepQuest, StatusQuest)
  ...
  if StepQuest==1 then
      if Player.Player_FLD_LEVEL<60 then 
          ...
          if QuestLevel==1 then  --neu dang o buoc 1
              ...
              local  bool909000001 = GetQuestItme(UserWorldId, 909000001, 1)  --kiem tra vat pham nhiem vu
              if bool909000001 then --du yeu cau
                  AddQuestItme(UserWorldId,909000001,0) --xoa item nhiem vu
      ...
      if Player.Player_FLD_LEVEL<60 then  
         ...
         if QuestLevel==2 then 
            SendMissionMsg(UserWorldId,QuestId, 22, StatusQuest) 
                  ...
                  AddQuestItme(UserWorldId,909000001,1) --them vat pham nhiem vu cho buoc tiep theo
            StatusQuest=1
      ...
      local controng=GetPackages(UserWorldId,1)  --vi tri con trong
        if controng.Count < 1 then  --neu thung do day 
          ...
          return 
                  ...
                  AddExpToCharacter(UserWorldId,300000) 
        end 
