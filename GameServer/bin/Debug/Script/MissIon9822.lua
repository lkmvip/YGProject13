...
--Ten nhiem vu: (½»»»Î´ÖªÒø±Ò) --- So buoc: (1)
function MissIon9822(UserWorldId,QuestId, StepQuest, StatusQuest)
  ...
  if StepQuest==1 then
      if Player.Player_FLD_LEVEL<1 then 
          ...
          if QuestLevel==1 then  --neu dang o buoc 1
              ...
              local  bool890000002 = GetQuestItme(UserWorldId, 890000002, 1)  --kiem tra vat pham nhiem vu
              if bool890000002 then --du yeu cau
                  AddQuestItme(UserWorldId,890000002,0) --xoa item nhiem vu
      ...
      if Player.Player_FLD_LEVEL<1 then  
         ...
         if QuestLevel==2 then 
            SendMissionMsg(UserWorldId,QuestId, 22, StatusQuest) 
                  ...
                  AddQuestItme(UserWorldId,890000002,1) --them vat pham nhiem vu cho buoc tiep theo
            StatusQuest=1
      ...
      local controng=GetPackages(UserWorldId,1)  --vi tri con trong
        if controng.Count < 1 then  --neu thung do day 
          ...
          return 
                  ...
                  AddMoneyToCharacter(UserWorldId,7000000) 
                  SendSysMsg(UserWorldId,"Nhan duoc 7000000 tien vang!", 9, "He thong")
        end 
