...
function MissIon532(UserWorldId,QuestId, StepQuest, StatusQuest)
  ...
  if StepQuest==1 then
      if Player.Player_FLD_LEVEL<45 then 
          ...
          if QuestLevel==1 then  --neu dang o buoc 1
              ...
              local  bool900000686 = GetQuestItme(UserWorldId, 900000686, 1)  --kiem tra vat pham nhiem vu
              if bool900000686 then --du yeu cau
                  AddQuestItme(UserWorldId,900000686,0) --xoa item nhiem vu
      ...
      if Player.Player_FLD_LEVEL<45 then  
         ...
         if QuestLevel==2 then 
            SendMissionMsg(UserWorldId,QuestId, 22, StatusQuest) 
                  ...
                  AddQuestItme(UserWorldId,900000686,1) --them vat pham nhiem vu cho buoc tiep theo
            StatusQuest=1
      ...
      local controng=GetPackages(UserWorldId,3)  --vi tri con trong
        if controng.Count < 3 then  --neu thung do day 
          ...
          return 
              ...
              --tang thuong
              local weiz1008000237=GetPackage(UserWorldId)  --vi tri con trong
                  AddItme(UserWorldId,1008000237,weiz1008000237,1) 
              --tang thuong
              local weiz1008000019=GetPackage(UserWorldId)  --vi tri con trong
                  AddItme(UserWorldId,1008000019,weiz1008000019,1) 
                  AddExpToCharacter(UserWorldId,420000) 
        end 
