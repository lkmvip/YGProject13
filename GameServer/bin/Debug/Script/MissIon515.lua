...
function MissIon515(UserWorldId,QuestId, StepQuest, StatusQuest)
  local Player = GetPlayer(UserWorldId)
--Cac buoc xu li nhiem vu
  if StepQuest==1 then
      if Player.Player_FLD_LEVEL<21 then 
          SendSysMsg(UserWorldId,"Ban phai dat cap do 21 moi co the nhan nhiem vu !", 9, "He thong")
        ...
        if controng.Count < 4 then  --neu thung do day 
          ...
          return 
              ...
              local weiz800000028=GetPackage(UserWorldId)  --vi tri con trong
                  ...
                  AddExpToCharacter(UserWorldId,6500) 
                  AddKiToCharacter(UserWorldId,500) 
                  AddMoneyToCharacter(UserWorldId,50000) 
                  SendSysMsg(UserWorldId,"Nhan duoc 50000 tien vang!", 9, "He thong")
        end 
