...
function MissIon438(UserWorldId,QuestId, StepQuest, StatusQuest)
  local Player = GetPlayer(UserWorldId)
--Cac buoc xu li nhiem vu
  if StepQuest==1 then
      if Player.Player_FLD_LEVEL<5 then 
          SendSysMsg(UserWorldId,"Ban phai dat cap do 5 moi co the nhan nhiem vu !", 9, "He thong")
      ...
      local controng=GetPackages(UserWorldId,2)  --vi tri con trong
        if controng.Count < 2 then  --neu thung do day 
          ...
          return 
                  ...
                  AddExpToCharacter(UserWorldId,250) 
                  AddMoneyToCharacter(UserWorldId,3000) 
                  SendSysMsg(UserWorldId,"Nhan duoc 3000 tien vang!", 9, "He thong")
        end 
