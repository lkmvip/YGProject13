...
--Ten nhiem vu: (达到目标!! 28级庆祝礼物) --- So buoc: (0)
function MissIon1213(UserWorldId,QuestId, StepQuest, StatusQuest)
  local Player = GetPlayer(UserWorldId)
--Cac buoc xu li nhiem vu
  if StepQuest==1 then
      if Player.Player_FLD_LEVEL<28 then 
          SendSysMsg(UserWorldId,"Ban phai dat cap do 28 moi co the nhan nhiem vu !", 9, "He thong")
      ...
      local controng=GetPackages(UserWorldId,1)  --vi tri con trong
        if controng.Count < 1 then  --neu thung do day 
          ...
          return 
              ...
              --tang thuong
              local weiz1007000009=GetPackage(UserWorldId)  --vi tri con trong
                  AddItme(UserWorldId,1007000009,weiz1007000009,1) 
        end 
