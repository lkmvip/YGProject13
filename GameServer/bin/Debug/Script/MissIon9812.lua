...
--Ten nhiem vu: (勇者的证明) --- So buoc: (1)
function MissIon9812(UserWorldId,QuestId, StepQuest, StatusQuest)
  ...
  if StepQuest==1 then
      if Player.Player_FLD_LEVEL<1 then 
          ...
          if QuestLevel==1 then  --neu dang o buoc 1
              ...
              local  bool999000563 = GetQuestItme(UserWorldId, 999000563, 1)  --kiem tra vat pham nhiem vu
              if bool999000563 then --du yeu cau
                  AddQuestItme(UserWorldId,999000563,0) --xoa item nhiem vu
      ...
      if Player.Player_FLD_LEVEL<1 then  
         ...
         if QuestLevel==2 then 
            SendMissionMsg(UserWorldId,QuestId, 22, StatusQuest) 
                  ...
                  AddQuestItme(UserWorldId,999000563,1) --them vat pham nhiem vu cho buoc tiep theo
            StatusQuest=1
