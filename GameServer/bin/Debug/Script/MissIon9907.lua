...
--Ten nhiem vu: (ÎäÑ«½±Àø»î¶¯(ÕýÅÉ)) --- So buoc: (0)
function MissIon9907(UserWorldId,QuestId, StepQuest, StatusQuest)
  local Player = GetPlayer(UserWorldId)
--Cac buoc xu li nhiem vu
  if StepQuest==1 then
      if Player.Player_Zx==2 then
          SendSysMsg(UserWorldId,"Ta phai khong the nhan nhiem vu !", 9, "He thong")
          SendMissionMsg(UserWorldId,QuestId, 12, StatusQuest)
      elseif Player.Player_Zx==0 then
          SendSysMsg(UserWorldId,"Chua gia nhap mon phai, khong the nhan nhiem vu !", 9, "He thong")
          SendMissionMsg(UserWorldId,QuestId, 12, StatusQuest)
      end
