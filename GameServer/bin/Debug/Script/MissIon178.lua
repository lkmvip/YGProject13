...
--Ten nhiem vu: (??? 5?? ??(??-1)) --- So buoc: (4)
function MissIon178(UserWorldId,QuestId, StepQuest, StatusQuest)
  ...
  if StepQuest==1 then
      if Player.Player_FLD_LEVEL<100 then 
          ...
          if QuestLevel==1 then  --neu dang o buoc 1
                  AddQuestItme(UserWorldId,900000515,1) --them vat pham nhiem vu cho buoc tiep theo
              StatusQuest=2 
              AddQuest(UserWorldId,QuestId, StatusQuest) 
              SendMissionMsg(UserWorldId,QuestId, 11, StatusQuest)  --cho lam buoc tiep theo la 2
          elseif QuestLevel==2 then
              local  bool900000515 = GetQuestItme(UserWorldId, 900000515, 1)  --kiem tra vat pham nhiem vu
              if bool900000515 then --du yeu cau
                  AddQuestItme(UserWorldId,900000515,0) --xoa vat pham nhiem vu
      ...
      local controng=GetPackages(UserWorldId,1)  --vi tri con trong
        if controng.Count < 1 then  --neu thung do day 
          ...
          return 
              ...
              AddQuestItme(UserWorldId,900000521,1) --them vat pham cho nhiem vu khac
        end 
